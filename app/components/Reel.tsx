...
//                     hovered === i ? "bg-black/45" : "bg-black/20"
//                   }`}
//                 />
...
export default function Reel({ data }: any) {
  const trackRef = useRef<HTMLDivElement | null>(null);
  const intervalRef = useRef<NodeJS.Timeout | null>(null);
  const indexRef = useRef(0); // ✅ use ref so interval always reads latest value
  const [hovered, setHovered] = useState<number | null>(null);
  const [posts, setPosts] = useState<any[]>([]);
  ...
  async function fetchReels() {
    try {
      const response = await fetch(
        "/backend/instagram?username=winsteadglobal"
      );
      ...
      const videoPosts =
        json?.result?.edges
          ?.filter(
            (item: any) =>
              item?.node?.video_versions &&
              item.node.video_versions.length > 0
          )
          .map((item: any) => ({
            id: item.node.id,
            code: item.node.code,
            ...
            thumbnail:
              ...
              videoUrl: item.node.video_versions?.[0]?.url,
  ...
  useEffect(() => {
    ...
    const getCardWidth = () => {
      ...
      return firstCard ? firstCard.offsetWidth + 16 : 250;
    };
    ...
    const animate = () => {
      ...
      gsap.to(trackRef.current, {
        x: -(moveWidth * indexRef.current),
        duration: 1.2,
        ease: "power3.inOut",
      });

      if (indexRef.current >= posts.length) {
        setTimeout(() => {
          if (!trackRef.current) return;
          gsap.set(trackRef.current, { x: 0 });
          indexRef.current = 0;
