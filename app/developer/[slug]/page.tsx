...
export default function DeveloperDetailsPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      const res = await WebsiteContentService.GetDeveloperCommunities(
        params.slug,
      );
      const res2 = await WebsiteContentService.getdeveloperProjects(
        params.slug,
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const found = data.find((item: any) => item.slug === params.slug);
      ...
      const mapped = found
        ? {
            ...
            category: "Luxury",
            bannerimage: found.data.bannerimage || found.data?.banner || "",
            type: "Developer",
            ...
            slug: found.slug,
            description:
              found.description?.trim() || "No description available",
          }
        : null;
      ...
      <main className="bg-black text-white">
              ...
              <div className="mt-5 text-lg leading-relaxed text-white">
                {
                  <ReadMoreSlider
                    description={
                      ...
                      developer.description.trim() !== "" // ← check trim
                        ? developer.description
                        : `Discover master-planned luxury communities crafted by ${developer.name}, where lifestyle, architecture, and long-term investment value come together in Dubai’s most desirable locations.`
                    }
        ...
        <section className="px-6 md:px-10 pb-24">
          <div className="mx-auto max-w-7xl">
            ...
            {!projects?.communities?.length && !standaloneprojects?.length && (
